...
`;
  ...
  const handleSendClick = async () => {
    ...
    try {
      ...
      const mergeResponse = await fetch('https://dev.patientime.ai/repair_and_merge_pdfs', {
        method: 'POST',
        body: formData,
      });
      ...
      if (mergeResponse.ok) {
        ...
        if (rewriteResponse.ok) {
          ...
          let cleanedString = rewrittenPdfString.replace(/^```json\s*\n/, '').replace(/\n```$/, '');
  ...
  const handleDownloadPdf = (data) => {
    ...
    const lineHeight = (pageHeight - 2 * margin) / maxLinesPerPage; // Dynamic line height for 26 lines
    let y = margin + lineHeight; // Initial Y position
    let currentPage = 1;
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(10);
      const textWidth = doc.getTextWidth("Physician Preferred");
      doc.setTextColor(150); // Light gray
      doc.text("Physician Preferred", pageWidth - margin - textWidth, margin);
    ...
    const renderContent = (data, indentation = 0) => {
      ...
      if (typeof data === "object" && data !== null) {
        Object.entries(data).forEach(([key, value]) => {
          ...
          doc.setFont("helvetica", "bold");
          doc.setFontSize(14);
          doc.text(`${" ".repeat(indentation)}${key}:`, margin, y);
          y += lineHeight;

          // Render value recursively
          renderContent(value, indentation + 4); // Increase indentation for nested content
        });
      } else {
        // Add value in normal font
        doc.setFont("helvetica", "normal");
        doc.setFontSize(12);
        doc.text(`${" ".repeat(indentation)}${String(data)}`, margin, y);
        y += lineHeight;
      }
    };
